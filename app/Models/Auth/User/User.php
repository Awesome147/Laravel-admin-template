...
class User extends Authenticatable
{
    ...
    public function  checkUser($field,$fieldValue){


        $data = DB::table('users')->join('users_roles','users.id','users_roles.user_id')->where('users.'.$field,$fieldValue)->where('users_roles.role_id',2)->get()->first();
        return $data;
